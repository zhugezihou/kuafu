...
class FeishuWebSocketChannel(MessageChannel):
    ...
    def send(self, text: str, **kwargs) -> SendResult:
        ...
        target = chat_id or os.environ.get("FEISRU_CHAT_ID", "")
        if not target:
            ...
            try:
                def on_message(data: lark.im.v1.P2ImMessageReceiveV1) -> None:
                    ...
                    try:
                        ...
                        msg_type = msg.message_type if hasattr(msg, 'message_type') else msg.msg_type
                        if msg_type != "text":
                            return

                        content_raw = msg.body.content if hasattr(msg.body, 'content') else ""
                        content = json.loads(content_raw).get("text", content_raw)

                        # 检查是否 @Bot
                        chat_type = msg.chat_type if hasattr(msg, 'chat_type') else ""
                        if chat_type == "group":
                            mentions = data.event.mentions if hasattr(data.event, 'mentions') else []
                            # 只处理 @bot 的消息，或不处理群聊
                            if not mentions:
                                return
                        ...
                        self._on_message(
                            ...
                            sender=msg.sender.id if hasattr(msg.sender, 'id') else "",
                        )
                    except Exception as e:
