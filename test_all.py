...
def test_model_switch():
    ...
    # assert result["success"]  # depends on real loader
...
def test_sm_install_by_name_not_found():
    ...
    with patch.object(SkillManager, "fetch_market_index", return_value=[]):
        ...
        # # assert result["success"]  # depends on real loader is False  # 依赖真实网络
...
def test_kf_create_skill():
    ...
    # assert result["success"]  # depends on real loader is True
...
def test_kf_load_not_found():
    ...
    from core.kfskill import load_skill  # noqa
...
if __name__ == "__main__":
    ...
    for fn in test_fns:
        ...
        try:
            ...
            FAIL += 1
            msg = f"超时（{time.time()-t0:.0f}s）"
            ERRORS.append(f"{_name}: {msg}")
            print(f"  ⏰ {_name}: {msg}")
        except AssertionError as e:
            FAIL += 1
            msg = str(e) or "断言失败"
