...
class TestSkillDelegation:
    """Cover L904, L909-935: skill delegation thread path."""

    def test_complex_skill_delegates_in_background(self, agent_loop, mock_triggers):
        """L909-935: complex skills trigger background delegation thread."""
        with patch("core.skill_resolver.match_skills") as mock_match, \
             patch("core.skill_resolver.resolve_skill_execution") as mock_resolve, \
             patch("core.skill_resolver.build_delegation_prompt", return_value="delegate prompt"), \
             patch("core.subagent.handle_delegate") as mock_delegate, \
             patch("core.agent_loop.threading.Thread") as mock_thread:

            mock_match.return_value = [{"name": "complex_skill"}]
            mock_resolve.return_value = ([], [{"name": "complex_skill", "description": "Complex task"}])
            mock_delegate.return_value = {"success": True, "summary": "done", "output": "works", "duration": 2.0}

            agent_loop.llm.chat.return_value = _llm_response(
                content="Task complete.",
                tool_calls=[_tool_call("finish", {"result": "done"})],
            )

            result = agent_loop.run("test complex task")

            # Verify delegation thread was created and started
            # Thread is called twice: once for delegation, once for async post-task
            assert mock_thread.call_count >= 1
            assert agent_loop._delegation_thread is not None
            assert result["success"]

    def test_complex_skill_local_backend_skips_delegation(self, agent_loop, mock_triggers):
        """Local backend should skip delegation even with complex skills."""
        agent_loop.llm.backend = "local"

        with patch("core.skill_resolver.match_skills") as mock_match, \
             patch("core.skill_resolver.resolve_skill_execution") as mock_resolve, \
             patch("core.skill_resolver.build_delegation_prompt") as mock_prompt:

            mock_match.return_value = [{"name": "complex_skill"}]
            mock_resolve.return_value = ([], [{"name": "complex_skill"}])

            agent_loop.llm.chat.return_value = _llm_response(
                content="done", tool_calls=[_tool_call("finish", {"result": "ok"})],
            )

            result = agent_loop.run("test")

            mock_prompt.assert_not_called()
            assert result["success"]

    def test_skill_delegation_failure_handled(self, agent_loop, mock_triggers):
        """L925: delegation failure sets error result."""
        with patch("core.skill_resolver.match_skills") as mock_match, \
             patch("core.skill_resolver.resolve_skill_execution") as mock_resolve, \
             patch("core.skill_resolver.build_delegation_prompt", return_value="prompt"), \
             patch("core.subagent.handle_delegate") as mock_delegate, \
             patch("core.agent_loop.threading.Thread") as mock_thread:

            mock_match.return_value = [{"name": "bad_skill"}]
            mock_resolve.return_value = ([], [{"name": "bad_skill"}])
            mock_delegate.return_value = {"success": False, "output": "failed", "duration": 0}

            agent_loop.llm.chat.return_value = _llm_response(
                content="done", tool_calls=[_tool_call("finish", {"result": "ok"})],
            )

            result = agent_loop.run("test fail delegate")
            assert result["success"]
