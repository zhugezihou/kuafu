...
class TestEvolutionTracker:
    ...
    def test_diff_skill_versions_same(self, tmp_path):
        ...
        tracker.record_skill_content("ds2", "same", "ds2.yaml")  # auto v1
        v = tracker.record_skill_evolution("ds2", "ds2_v2.yaml")  # v2
        tracker.record_skill_content("ds2", "same", "ds2_v2.yaml", version=v)  # v2
        diff = tracker.diff_skill_versions("ds2", 1, v)
        assert diff is not None
        tracker.close()
...
class TestFitnessEvaluator:
    ...
    def test_evaluate_recency_none(self):
        from core.gepa_engine import FitnessEvaluator
        record = FitnessEvaluator.evaluate()
        assert record.metrics["recency"] == 0.5
...
class TestGEPAEngine:
    ...
    def test_crossover(self):
        ...
        child = engine.crossover(g_a, g_b)
        assert child is not None
...
class TestEvolutionState:
    ...
    def test_init_with_json_migration(self, tmp_path):
        ...
        assert state_path.with_name(state_path.name + ".bak").exists()
    ...
    def test_record_error(self, tmp_path):
        from core.evolution_state import EvolutionState
        state = EvolutionState(root_dir=tmp_path)
        state.record_error("UniqueTestErrorConnectionTimeout")
        assert state.is_unknown_error("UniqueTestErrorConnectionTimeout") is False
        assert state.is_unknown_error("SomeOtherNewError") is True
...
class TestAgentLoopDeep:
    ...
    def test_run_with_empty_llm_response_key(self):
        ...
        loop = self._make_loop()
        loop.llm.chat.return_value = {"success": True}  # missing content, tool_calls
        result = loop.run(task="test")
        assert result["success"] is True
    ...
    def test_build_system_prompt_with_quality(self):
        ...
        loop = self._make_loop()
        with patch('core.agent_loop.get_quality', return_value=["质量要求1", "质量要求2"]):
...
class TestApprovalDeep:
    ...
    def test_deny_rules_invalid_regex_fallback(self):
        ...
        DenyRules._rules = []
        DenyRules.save()
        # Pattern that's not a valid regex (special chars)
...
class TestGatewayHandler:
    ...
    def _make_handler(self, method="GET", path="/health", body=b"",
        ...
        # Reset class vars
        GatewayHandler.agent = agent or MagicMock()
    ...
    def test_do_GET_auth_failure(self):
        ...
        with patch.object(type(handler), '_check_auth', return_value=False) as mock_auth:
            handler.do_GET = MagicMock()
            # We just test that _check_auth is called
            handler._check_auth()
    ...
    def test_get_404(self):
        ...
        # _check_auth returns True
        with patch.object(type(handler), '_check_auth', return_value=True):
            handler.do_GET()
    ...
    def test_batch_submit_success(self):
        ...
        with patch.object(handler, '_read_body', return_value={"tasks": ["task1", "task2"]}):
            with patch("core.gateway.BatchEngine", return_value=mock_engine):
                handler._handle_batch_submit()
    ...
    def test_cron_create_no_scheduler(self):
        ...
        with patch.object(handler, '_read_body', return_value={"name": "test", "schedule": "30m", "task": "hello"}):
            with patch("core.gateway.CronScheduler") as MockCS:
...
class TestCronScheduler:
    ...
    def test_execute_task_no_callback(self):
        ...
        from core.cron_scheduler import CronScheduler, CronTask
        cs = CronScheduler()  # No on_task_run
    ...
    def test_load_config_simple_yaml(self):
        ...
        try:
            cs = CronScheduler()
            cs._config_path = Path(fpath)
            # Simulate yaml import error
            with patch("core.cron_scheduler.yaml", None):
...
class TestHooks:
    ...
    def test_hook_registry_init_empty(self):
        ...
        with patch.object(HOOKS_CONFIG_PATH, 'exists', return_value=False):
            HookRegistry.init()
    ...
    def test_hook_registry_init_from_config(self):
        ...
        with patch.object(HOOKS_CONFIG_PATH, 'exists', return_value=True):
            with patch.object(HOOKS_CONFIG_PATH, 'read_text', return_value=json.dumps(config_data)):
                HookRegistry.init()
                assert HookRegistry._initialized is True
    ...
    def test_hook_registry_init_bad_config(self):
        ...
        from core.hooks import HookRegistry, HOOKS_CONFIG_PATH
        with patch.object(HOOKS_CONFIG_PATH, 'exists', return_value=True):
            with patch.object(HOOKS_CONFIG_PATH, 'read_text', return_value="not valid json!!!"):
                HookRegistry.init()
                assert HookRegistry._initialized is True
    ...
    def test_trigger_async(self):
        ...
        with patch("core.hooks.threading.Thread") as mock_thread:
    ...
    def test_trigger_sync(self):
        ...
        with patch("core.hooks.trigger") as mock_trigger:
            ...
            mock_trigger.assert_called_with("on_agent_start", synchronous=True)
...
class TestIdentity:
    ...
    def test_load_identity_statement_exists(self):
        ...
        with patch.object(IDENTITY_PATH, 'exists', return_value=True):
            with patch.object(IDENTITY_PATH, 'read_text', return_value="I am Kuafu."):
    ...
    def test_load_identity_statement_not_exists(self):
        ...
        from core.identity import load_identity_statement, IDENTITY_PATH
        with patch.object(IDENTITY_PATH, 'exists', return_value=False):
            result = load_identity_statement()
            assert "夸父" in result
    ...
    def test_load_identity_statement_caching(self):
        ...
        from core.identity import load_identity_statement, IDENTITY_PATH
        with patch.object(IDENTITY_PATH, 'exists', return_value=True):
            with patch.object(IDENTITY_PATH, 'read_text', return_value="Version 1"):
...
class TestAgentLoopExtra:
    ...
    def test_quality_score_high_error_rate(self):
        ...
        quality = loop._quality_score(task_result, messages)
        detail = " ".join(quality["detail"])
        assert "错误率" in detail or "错误" in detail
